...
MAX_VALUE_TO_BE_CONNECTED = 19.8 # Calculated by taking the average of the ToA value differences of a large number
# of blobs in frames
...
class BlobFinder:
    ...
    def add(self, xyc):
        (x, y, c) = xyc
        self.blob.append((x, y, c))
        close_region = self.circle((x, y))
        prev = c
        for (x1, y1) in close_region:
            # Check if the C value is ± our constant and c (to make sure it's a part of the cluster in both space and time)
            if self.frame[x1][y1] > 0 and prev - MAX_VALUE_TO_BE_CONNECTED <= self.frame[x1][y1] <= prev + MAX_VALUE_TO_BE_CONNECTED:
                c1 = self.frame[x1][y1]
                prev = c1
                self.frame[x1][y1] = 0
